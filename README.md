...
Simple Angular 1.5 bootstrap with Gulp and Bootstrap. [Heroku deployment](https://simple-angular-bootstrap.herokuapp.com)
...
### gulpfile.js
This is the ```package.json``` for Gulp which means whenever you type ```gulp <script>``` it always goes through here. So to separate the scripts there is a small script that runs through the ```gulp``` folder and requires each and everyone of the ```.js````-files. Particularly bad practise would be to have all your gulp scripts inside this file since it makes managing of them quite hard after you've gathered up a few.
...
## directives
Aret the Angular version of Web components. If that doesn't say anything think about HTML-elements. You have stuff like ```<div>```, ```<button>``` and ```<input>``` etc. What if you wanted to create your own custom HTML-element? Like for example ```<my-directive>```. And that component will its own separate logic how it works and how it looks and you can use it anywhere inside your HTML like a regular element. That's basically what directives are, custom HTML-elements. They might look a bit confusing but they work very well. You can inject your services to its scope or maybe create a nested directive with other directive inside it. It's cool and if you ever switch up to React and Angular 2.0 you'll find them there too, in a different maybe, but they'll be there.

## factories
Eh not really too excited about these. If you want to use classes inside your app for some reason you should try using these. Other than that, I'd keep away from them.
